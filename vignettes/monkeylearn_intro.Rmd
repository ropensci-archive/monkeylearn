...
monkey_extract(text, extractor_id = "ex_y7BPYzNG", unnest = TRUE)
...
monkey_classify(request, classifier_id = "cl_oFKL5wft", unnest = TRUE)
...
monkey_classify(request, classifier_id = "cl_oJNMkt2V", unnest = TRUE)
...
request <- c(text1, text2)
monkey_classify(request, classifier_id = "cl_KFXhoTdt", unnest = TRUE)
...
request <- c(text1, text2)
monkey_classify(request, classifier_id = "cl_5icAVzKR", unnest = TRUE)
...
Let's take a look at the `resp` output column. 
```{r}
output$resp
```

You can easily choose an unnested output by setting the **unnest flag** to TRUE to get one row per classification/extraction. 

```{r unnest_true}
(output_unnested <- monkey_classify(input, unnest = TRUE, verbose = FALSE))
```

We could have gotten the same result by sending in a dataframe and a named column.
...
A `r round(speedup, digits=0)`x speedup isn't so bad! Worth keeping in mind that if you need the blazing fast speeds you might consider upgrading to a higher MonkeyLearn price tier. 
